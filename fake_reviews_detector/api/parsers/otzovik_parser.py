...
def fetch_otzovik_reviews(product_url: str) -> List[Dict]:
    ...
    try:
        ...
        DebugLog.objects.create(  # Сохраняем HTML в DebugLog
            html_content=response.text, success=True
        )
        ...
        # На основании анализа debug_otzovik.html подберите актуальные
        # селекторы.
        ...
        for idx, item in enumerate(items):
            try:
                ...
                reputation = int(
                    reputation_elem.text) if reputation_elem else 0

                if not text_elem or not rating_elem:
                    logger.debug(
                        f"Review #{idx} пропущен: отсутствует текст или рейтинг"
                    )
                    continue
                ...
                try:
                    ...
                    logger.debug(
                        f"Review #{idx} ошибка преобразования рейтинга: {rating_raw}"
                    )
                    continue

                logger.debug(
                    f"Review #{idx}: Rating {rating}, Text: {text[:50]}...")
