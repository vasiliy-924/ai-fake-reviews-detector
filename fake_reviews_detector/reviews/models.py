...
class Review(models.Model):
    ...
    meta = models.JSONField(
        default=dict, verbose_name="Метаданные"
    )  # URL, дата парсинга и т.д.
...
class AnalysisResult(models.Model):
    review = models.OneToOneField(
        Review, on_delete=models.CASCADE, related_name="analysis", verbose_name="Отзыв"
    )
    is_fake = models.BooleanField(verbose_name="Фейковый")
    probability = models.FloatField(verbose_name="Вероятность")
    model_version = models.CharField(
        max_length=50, default="v1.0", verbose_name="Версия модели"
    )
